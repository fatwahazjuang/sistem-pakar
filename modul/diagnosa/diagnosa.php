...
switch ($_GET['act']) {
  ...
  default:
    if ($_POST['submit']) {
      $arcolor = array('#ffffff', '#ff0000', '#019AFF', '#00CBFD', '#00FEFE', '#A4F804','#ffffff');
      ...
      $np1 = 0;
      foreach ($argejala as $key1 => $value1) {
        $np1++;
        $totalgejala += $value1;
      }
      //print_r($totalgejala);
  ...
  // --------------------- END -------------------------
    ...
    <h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
        ...
        echo "</table><h3 class='mb4 text-center'>Hasil Diagnosa</h3>"; 
        echo "<div class='callout callout-default'>  <b><h3 class='text text-success text-center'>" . $nmpkt[2] . "</b> / " . round($vlpkt[2] * 100, 2) . " % (" . $vlpkt[2] . ")<br></h3></div></div>";
        if($vlpkt[2] * 100 > 0 && $vlpkt[2] * 100 < 50 ){
          echo "<div class='alert alert-success text-center' role='alert'><h4>Tidak Ada Indikasi Kusta</h4></div>";
        }else if($vlpkt[2] * 100 >= 50 && $vlpkt[2] * 100 < 80){
          echo "<div class='alert alert-warning text-center' role='alert'><h4>Pantau dalam 3 bulan dan cek ulang!!!  Serta menerapkan pola hidup sehat seperti rutin berolahraga, konsumsi makanan sehat, dan tidur yang cukup, untuk menjaga imunitas tubuh Anda</h4></div>";
        }else if($vlpkt[2] * 100 >= 80){
          echo "<div class='alert alert-danger text-center' role='alert'><h4>Segeralah periksakan kesehatan Anda ke Pusat pelayanan kesehatan terdekat agar Anda mendapatkan penanganan medis secepat mungkin.</h4></div>";
        }else{
          echo "<div class='alert alert-success text-center' role='alert'><h4>Tidak Ada gejala Yang dipilih</h4></div>";
        }
  ...
  // --------------------- END -------------------------
    ...
    <h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
        ...
        echo "</table><h3 class='mb4 text-center'>Hasil Diagnosa</h3>"; 
        echo "<div class='callout callout-default'>  <b><h3 class='text text-success text-center'>" . $nmpkt[1] . "</b> / " . round($vlpkt[1] * 100, 2) . " % (" . $vlpkt[1] . ")<br></h3></div></div>";
        if($vlpkt[1] * 100 > 0 && $vlpkt[1] * 100 < 50 ){
          echo "<div class='alert alert-success text-center' role='alert'><h4>Tidak Ada Indikasi Kusta</h4></div>";
        }else if($vlpkt[1] * 100 >= 50 && $vlpkt[1] * 100 < 80){
          echo "<div class='alert alert-warning text-center' role='alert'><h4>Pantau dalam 3 bulan dan cek ulang!!!  Serta menerapkan pola hidup sehat seperti rutin berolahraga, konsumsi makanan sehat, dan tidur yang cukup, untuk menjaga imunitas tubuh Anda</h4></div>";
        }else if($vlpkt[1] * 100 >= 80){
          echo "<div class='alert alert-danger text-center' role='alert'><h4>Segeralah periksakan kesehatan Anda ke Pusat pelayanan kesehatan terdekat agar Anda mendapatkan penanganan medis secepat mungkin.</h4></div>";
        }else{
          echo "<div class='alert alert-success text-center' role='alert'><h4>Tidak Ada gejala Yang dipilih</h4></div>";
        }
	 ...
	 <div class='alert alert-success alert-dismissible'>
                ...
                Silahkan memilih gejala sesuai dengan kondisi Anda, jika sudah tekan tombol proses (<i class='fa fa-search-plus'></i>)  di bawah untuk melihat hasil.
              </div>
      ...
      while ($r3 = mysqli_fetch_array($sql3)) {
        ...
        echo '<td class="opsi"><select name="kondisi[]" id="sl' . $i . '" class="opsikondisi"/ required><option data-id="0" value="">Pilih jika sesuai</option>';
